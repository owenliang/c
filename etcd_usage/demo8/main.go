...
func demo8() {
	...
	// 如果key不存在
	txn.If(clientv3.Compare(clientv3.CreateRevision("/lock/job8"), "=", 0)).
		// 则抢占建立
		Then(clientv3.OpPut("/lock/job8", "I am demo8", clientv3.WithLease(leaseId))).
		// 否则乐观锁抢占失败, 获取其值
		Else(clientv3.OpGet("/lock/job8"))
